...
export function InferenceReviewPanel({ groundId, inferences }: Props) {
  ...
  return (
    <div style={{ marginTop: 24, border: '1px solid var(--gw-amber-b)', borderRadius: 12, overflow: 'hidden' }}>
      {/* Header */}
      <button
        onClick={() => setExpanded(v => !v)}
        style={{
          width: '100%', background: 'var(--gw-amber-bg)', border: 'none', padding: '12px 16px',
          display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          cursor: 'pointer', textAlign: 'left',
        }}
      >
        <div>
          <span style={{ fontSize: 10.5, letterSpacing: '.09em', textTransform: 'uppercase', fontWeight: 700, color: 'var(--gw-amber-t)' }}>
            Inferences we made
          </span>
          <span style={{ fontSize: 11, color: 'var(--gw-amber-t)', marginLeft: 8, opacity: 0.7 }}>
            {active.length} item{active.length !== 1 ? 's' : ''}
          </span>
        </div>
        <span style={{ fontSize: 11, color: 'var(--gw-amber-t)', opacity: 0.6 }}>{expanded ? '▲ hide' : '▼ review'}</span>
      </button>

      {expanded && (
        <div style={{ padding: '4px 0 8px' }}>
          <p style={{ fontSize: 12, color: 'var(--gw-sub)', padding: '8px 16px 12px', margin: 0, lineHeight: 1.6 }}>
          ...
          {active.map((inf) => (
            <div key={inf.id} style={{ padding: '10px 16px', borderTop: '1px solid #F0E8D8' }}>
              <p style={{ fontSize: 13, color: 'var(--gw-text)', lineHeight: 1.55, margin: '0 0 4px' }}>
                "{inf.text}"
              </p>
              <p style={{ fontSize: 11, color: 'var(--gw-muted)', margin: '0 0 10px', lineHeight: 1.5 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button
                  ...
                  style={{
                    fontSize: 12, fontWeight: 600, color: 'var(--gw-navy)',
                    background: 'var(--gw-blue-bg)', border: '1px solid var(--gw-blue-b)',
                    borderRadius: 7, padding: '5px 12px', cursor: 'pointer',

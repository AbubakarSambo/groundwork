...
export function GroundParticipantPage() {
  ...
  const isClosed = ground.status === 'CLOSED' || ground.status === 'RESOLVED'

  // Find this user's participant record and their check-ins only
  ...
  const tabs: Tab[] = ['checkin', 'record', 'docs', 'report', ...(isClosed ? ['profile' as Tab] : [])]
      ...
      <div style={{ position: 'sticky', top: 0, zIndex: 50, background: 'var(--gw-bg)', borderBottom: '0.5px solid var(--gw-border)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px' }}>
          ...
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--gw-navy)' }}>{conf}/5</div>
          ...
          <div>
            {sessionOpen ? (
              <div>
                ...
                <button
                  onClick={() => navigate(`/checkin/${myCheckIn.id}`, {
                    state: {
                      sessionNumber: myCheckIn.sessionNumber,
              ...
              <div style={{ background: 'var(--gw-green-bg)', border: '0.5px solid var(--gw-green-b)', borderRadius: 8, padding: 14 }}>
                ...
                <div style={{ fontSize: 12, color: 'var(--gw-sub)', lineHeight: 1.6 }}>Your contribution has been recorded. The administrator releases the report once both parties complete their sessions.</div>
              </div>
            )}
          </div>
        )}
        ...
        {tab === 'report' && (
          <div>
            <div style={{ textAlign: 'center', padding: '20px 0 16px' }}>
              <div style={{ fontSize: 20, fontWeight: 800, color: 'var(--gw-navy)' }}>{conf}/5</div>
              <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--gw-sub)', marginTop: 2 }}>{bl}</div>
            </div>

            {report?.releasedAt ? (
              <div>
                {/* Participant-scoped report: assumptions, clarity, questions */}
                {report.assumptions && report.assumptions.length > 0 && (
                  <div style={{ background: 'var(--gw-bg)', border: '0.5px solid var(--gw-border)', borderRadius: 10, padding: 16, marginBottom: 12 }}>
                    <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--gw-sub)', textTransform: 'uppercase', letterSpacing: '.04em', marginBottom: 10 }}>Assumptions you are carrying</div>
                ...
                {report.clarity && report.clarity.length > 0 && (
                  <div style={{ background: 'var(--gw-blue-bg)', border: '0.5px solid var(--gw-blue-b)', borderRadius: 10, padding: 16, marginBottom: 12 }}>
                    <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--gw-navy)', textTransform: 'uppercase', letterSpacing: '.04em', marginBottom: 10 }}>Where you have clarity</div>
                    <ul style={{ listStyle: 'disc', paddingLeft: 18, margin: 0 }}>
                ...
                {report.questions && report.questions.length > 0 && (
                  <div style={{ background: 'var(--gw-bg)', border: '0.5px solid var(--gw-border)', borderRadius: 10, padding: 16, marginBottom: 12 }}>
                    <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--gw-sub)', textTransform: 'uppercase', letterSpacing: '.04em', marginBottom: 10 }}>Questions to carry into the conversation</div>
                    <ul style={{ listStyle: 'disc', paddingLeft: 18, margin: 0 }}>
                      {report.questions.map((q: string, i: number) => <li key={i} style={{ fontSize: 13, lineHeight: 1.65, marginBottom: 5 }}>{q}</li>)}
                    </ul>
                  </div>
                )}
                ...
                {report.pattern && (
                  <div style={{ background: 'var(--gw-navy)', color: 'white', borderRadius: 10, padding: 16, marginBottom: 14 }}>
                    <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: '.08em', textTransform: 'uppercase', color: 'rgba(255,255,255,.55)', marginBottom: 8 }}>What your record reveals</div>
                    <div style={{ fontSize: 14, lineHeight: 1.65 }}>{report.pattern}</div>
                  </div>
                )}

                {/* Fallback for older report shape */}
                {!report.assumptions && !report.pattern && report.sharedPicture && (
                  <div style={{ background: 'var(--gw-navy)', color: 'white', borderRadius: 10, padding: 16, marginBottom: 14 }}>
                ...
                {report.privacy && (
                  <div style={{ fontSize: 12, color: 'var(--gw-sub)', lineHeight: 1.6, marginTop: 12, padding: '10px 12px', background: 'var(--gw-bg)', borderRadius: 7, border: '0.5px solid var(--gw-border)' }}>
                    {report.privacy}
                  </div>
                )}
              ...
              <div style={{ background: 'var(--gw-bg)', border: '0.5px solid var(--gw-border)', borderRadius: 10, padding: 16, marginBottom: 14 }}>
                ...
                <div style={{ fontSize: 12, color: 'var(--gw-sub)', lineHeight: 1.6 }}>The report is released by the ground administrator once both parties have completed their sessions. You will both see it at the same time — neither party reads it before the other.</div>
              </div>
            )}
          </div>
        )}

        {/* MY PROFILE — only shown on closed grounds */}
        {tab === 'profile' && isClosed && (
          <div>

...
**H1 is now GREEN (Phase 2 landed, `cda499d`)** — the "do not re-ask" instruction (FIX A) plus the claim-verification rule (FIX B) were added to `ENGINE_RULES` below HUMAN FIRST / above the pack agenda. H1 flipped red to green and no other tripwire reddened.

- **FIX B (verification): proven behaviorally** at real gemini-2.5-pro output on the REALIGN_TEAM thin pack. Before: accepts "everyone is aligned, morale is high, on track" and jumps to the pack tension question. After: "Good to hear things feel aligned and on track. When you say on track for the numbers, which specific number is everyone tracking toward?" - warm, then verifies. Consistent across runs.
- **FIX A (no re-ask): rests on the structural proof + tripwire, not a behavioral transcript.** A good-faith hunt for a real reproduction (four mechanism-aligned REALIGN_TEAM multi-turn histories: drift-question-answered-early, opener-then-content-free, hope-question-then-minimal, whole-pack-walked-then-dead-end; ~7 generations) did NOT get the pre-fix model to re-ask an already-answered question - it built forward or reframed every time. The re-ask the exec saw is intermittent (model sampling) or tied to a surface not reproduced here. FIX A is proven by: the instruction was genuinely absent (`no-reask-instruction=false`) and is now present (`true`), which the H1 tripwire encodes red->green. We do not claim a behavioral before/after for FIX A because we could not honestly produce one. Not-yet-guarded: clarification-session opener (needs a report+inference fixture), client `SESSION_END_PATTERNS` (vitest, cross-runner — the 4th end-detection list, noted in D).

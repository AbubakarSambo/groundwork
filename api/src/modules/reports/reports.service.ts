...
const POST_REPORT_GUIDE_SCHEMA = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      recommendedNextStep: {
        type: 'string',
        description: "A recommendation for THIS party alone, if one is warranted by their own record and the shared synthesis - e.g. 'additional support may help here', 'this may be worth escalating', 'a scope conversation may be due'. This is not a verdict on the person and it is never derived from another party's record. Empty string if nothing specific is warranted beyond the opening line and question.",
      },
    },
    required: ['openingLine', 'questionToCarry', 'toAcknowledge', 'recommendedNextStep'],
  },
};

const POST_REPORT_GUIDE_PROMPT =
  'You are Groundwork. A shared report has just been released to both parties. Given one party\'s record entries and the shared synthesis, produce a short, specific post-report guide for THIS party only. Four things: (1) one opening line they can use to start the real conversation - grounded, not defensive; (2) one question to carry into the room - genuine, not a challenge; (3) one concrete thing from the other side\'s record they should acknowledge, even if they see it differently; (4) recommendedNextStep - a personal recommendation for this party alone if the record warrants one (e.g. more support, an escalation, a scope conversation), grounded only in their own record and the shared synthesis, never a verdict on them as a person, and left empty if nothing specific applies. Brief, direct - no more than 3 sentences per item.';
...
const REPORT_SCHEMA = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      leadCalibrationNote: {
        type: 'string',
        description: "If the divergences in this report trace back to an ambiguous, missing, or under-specified opening brief from the initiator/lead - rather than a genuine disagreement between parties - name that explicitly here. Empty string if the brief was clear and the gap is not attributable to how the ground was set up.",
      },
    },
...
13. NEVER INVENT PARTY COUNTS OR ROLES. The PARTY ROSTER at the top of this corpus is the exhaustive, exact list of who is on this ground - use its exact count and exact labels only. Never state a number of parties, an "other parties" count, or a role/title/affiliation (e.g. "founder", "funders", "the board") that does not appear verbatim in the roster. If you are unsure how many parties are missing or who they are, use the roster's own wording rather than describing them yourself.`;
    ...
    const enrichedEngagement = {
      ...
      leadCalibrationNote: result.leadCalibrationNote ?? '',
    };
  ...
  private async generatePostReportGuides(
    ...
    const guides: Record<string, { openingLine: string; questionToCarry: string; toAcknowledge: string; recommendedNextStep: string }> = {};
    ...
    await Promise.all(
      participantIds.map(async (participantId) => {
        try {
          ...
          const result = await this.anthropic.extract<{ openingLine: string; questionToCarry: string; toAcknowledge: string; recommendedNextStep: string }>(
